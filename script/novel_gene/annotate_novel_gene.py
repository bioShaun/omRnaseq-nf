...
def swissprot_annotation(input_file, gene2tr,
    ...
    orf_cmd = f'TransDecoder.LongOrfs -t {input_file} --gene_trans_map {gene2tr}'
    ...
    blast_cmd = f'blastp -query {orf_file} -db {DB_LOCATION[db]} \
...
-outfmt "6 qseqid sseqid pident evalue bitscore stitle"'
    ...
    tr_map_df = pd.read_table(gene2tr, header=None,
                              names=['gene_id'], index_col=1)
    blast_df = pd.read_table(blast_out, header=None,
                             names=BLAST_HEADER)
    blast_df.loc[:, 'gene_id'] = [orfid2geneid(each, tr_map_df)
                                  for each in blast_df.qseqid]
    ...
    best_blast_df.to_csv(anno_file, sep='\t', index=False,
                         columns=['gene_id', 'gene_name', 'description'],
                         na_rep='--', header=False)
