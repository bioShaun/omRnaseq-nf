    ...
    for n, each_gene in enumerate(genes):
        ...
        if each_gene in blasttab_df.index:
            ...
            if diff_label:
                ...
                diff_stat = 'Constant'
        diff_stats.append(
            f'{each_analysis_gene}|{ori_gene}|{diff_stat}')
...
def title_id_attr(tags, blasttab_df, diff_df, diff_cutoff):
    ...
    for each_tag in tags:
        diff_stats, id_labes = new_title_stats(
            each_tag, blasttab_df, diff_df, diff_cutoff)
        if id_labes:
            if len(id_labes) == 1:
...
def displaty_area_tags(raw_html, blasttab_df, diff_df,
                       diff_cutoff):
...
def pathway_html(raw_html, blasttab_df, diff_df, outfile,
                 logfc_cutoff=1,
                 qvalue_cutoff=0.05):
    diff_cutoff = {'logfc_cutoff': logfc_cutoff,
...
def pathway_results(pathway_db, kegg_table, blasttab,
                    diff_table, outdir,
                    logfc_cutoff=1,
                    qvalue_cutoff=0.05):
    diff_cutoff = {'logfc_cutoff': logfc_cutoff,
    ...
    blasttab_df = pd.read_csv(blasttab, header=None,
                              index_col=1, names=BLAST_COLS,
                              sep='\t')
    ...
    # generate html files
    raw_htmls = [pathway_db / f'html/ssc/{pathway}.html'
                 for pathway in kegg_df.ID]
    pngs = [pathway_db / f'png/ssc/{pathway}.png'
            for pathway in kegg_df.ID]
    for n, raw_html in enumerate(raw_htmls):
        outfile = outdir / f'{raw_html.name}'
        if pathway_html(raw_html, blasttab_df, diff_df, outfile):
            # cp pngs
